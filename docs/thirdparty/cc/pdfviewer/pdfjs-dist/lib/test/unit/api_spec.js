...
describe("api", function () {
  ...
  describe("PDFDocument", function () {
    ...
    it("write a value in an annotation, save the pdf and check the value in xfa datasets (1)", async function () {
      ...
      let loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("issue16081.pdf"));
      let pdfDoc = await loadingTask.promise;
      const value = "Hello World";
    ...
    it("write a value in an annotation, save the pdf and check the value in xfa datasets (2)", async function () {
      ...
      let loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("f1040_2022.pdf"));
      let pdfDoc = await loadingTask.promise;
    ...
    it("write a a new annotation, save the pdf and check that the prev entry in xref stream is correct", async function () {
      ...
      let loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("bug1823296.pdf"));
      let pdfDoc = await loadingTask.promise;
  ...
  describe("Page", function () {
    ...
    it("gets annotations containing GoToE action (issue 8844)", async function () {
      const loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("issue8844.pdf"));
      const pdfDoc = await loadingTask.promise;
      const pdfPage = await pdfDoc.getPage(1);
      const annotations = await pdfPage.getAnnotations();
      expect(annotations.length).toEqual(1);
      expect(annotations[0].annotationType).toEqual(_util.AnnotationType.LINK);
      const {
        filename,
        content
    ...
    it("gets text content", async function () {
      ...
      expect((0, _util.objectSize)(styles)).toEqual(5);
...
page 1 / 3`);
    ...
    it("gets text content, with no extra spaces (issue 16119)", async function () {
      ...
      const loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("issue16119.pdf"));
      const pdfDoc = await loadingTask.promise;
      const pdfPage = await pdfDoc.getPage(1);
      const {
        items
...
Caron Broadcasting, Inc., an Ohio corporation (“Lessee”).`)).toEqual(true);
    ...
    it("gets text content with multi-byte entries, using predefined CMaps (issue 16176)", async function () {
      const loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("issue16176.pdf", {
        cMapUrl: _test_utils.CMAP_URL,
    ...
    it("gets text content with a rised text", async function () {
      const loadingTask = (0, _api.getDocument)((0, _test_utils.buildGetDocumentParams)("issue16221.pdf"));
      const pdfDoc = await loadingTask.promise;
      const pdfPage = await pdfDoc.getPage(1);
      const {
        items
  ...
  describe("PDFDataRangeTransport", function () {
    ...
    it("should fetch document info and page, without range, " + "using complete initialData", async function () {
      ...
      subArrays.push(initialData);
      const transport = new _api.PDFDataRangeTransport(data.length, initialData, true);
      transport.requestDataRange = function (begin, end) {
        fetches++;
      };
      const loadingTask = (0, _api.getDocument)({
