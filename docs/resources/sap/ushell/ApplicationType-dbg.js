...
sap.ui.define([
    ...
    function generateWCFResolutionResult (oMatchingTarget, sBaseUrl, fnExternalSystemAliasResolver) {
        var oUri = new URI(sBaseUrl),
            ...
            oEffectiveParameters = deepExtend({}, oMatchingTarget.mappedIntentParamsPlusSimpleDefaults),
            sSapSystem,
            sSapSystemDataSrc;
    ...
    function getInnerAppRoute (oTemplatePayload) {
        var sInnerAppRoute = undefined,
            sHashFragment = window.hasher.getHash(),
            indexOfInnerRoute = sHashFragment.lastIndexOf("&/"),
            iOffset = 1; //to avoid the starting "&";
        ...
        if (indexOfInnerRoute > 0) {
            ...
            try {
                ...
                Log.warning("inner route should be double encoded", e, "sap.ushell.sap.ushell.ApplicationType.getInnerAppRoute");
            }
        }
        ...
        return new Promise(function (fnResolve) {
            Promise.all([
                ...
                var oUserInfoService = aResults[0],
                    oPluginsService = aResults[1],
                    sUi5Version = aResults[2];
    ...
    */
    ...
    function generateURLTemplateResolutionResult (oMatchingTarget, sBaseUrl, fnExternalSystemAliasResolver) {
        return new Promise(function (fnResolveGlobal) {
            var oInbound = oMatchingTarget.inbound,
                oTemplateContext = oInbound.templateContext,
                oCapabilities = oTemplateContext.payload.capabilities || {};
    ...
    function handleURLTransformation (sUrl, oCapabilities, oTemplateContext) {
        return new Promise(function (fnResolve) {
            ...
            var bEnableTransfornation = isTransformationEnabled(oTransformation, oTemplateContext);
            if (bEnableTransfornation === true) {
                oIframeURI = new URI(sUrl);
                oService = oTransformation.transformations[0].service.uri;
                ...
                sServiceUrl = URI.expand("{+rootPath}/{+resourcePath}{?queryParams*}", {
                ...
                sap.ui.require(["sap/ui/thirdparty/datajs"], function (OData) {
                    OData.read({
                        ...
                        function (oRes) {
                            ...
                            if (oTransformation.transformations[0].sourceURLComponent === undefined) {
                                oTransformation.transformations[0].sourceURLComponent = "query";
                            }

                            if (oTransformation.transformations[0].sourceURLComponent === "query" || oTransformation.transformations[0].sourceURLComponent === "fragment") {
                                sUrl = oIframeURI[oTransformation.transformations[0].sourceURLComponent].apply(oIframeURI, [resVal]).toString();
                            } else {
                                Log.error(
                                    "The " + oTransformation.transformations[0].sourceURLComponent + " component of the URL in URI.js is not transformed",
                                    "",
                                    "sap.ushell.ApplicationType"
                                );
                            }
        ...
        var oTransformData = URLTemplateProcessor.prepareExpandData(
            {
                urlTemplate: "",
                parameters: {
                    names: {enabled: oTransformation.enabled}
                }
            },
            {},
            {},
