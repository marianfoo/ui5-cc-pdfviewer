...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.components.shell.Settings.userDefaults.controller.UserDefaultsSetting", {
        ...
        _createOdataModelBinding: function (oParameter, oJsonModel) {
            ...
            return oODataServiceData.metadata.then(function () {
                var oModelBind = {
                    isOdata: true,
        ...
        _getODataServiceData: function (sUrl, oFormattedParameter) {
            if (!this.oModelRecords[sUrl]) {
                ...
                this.oModelRecords[sUrl] = {
                    attachedListeners: [],
                    model: oModel,
                    metadata: new Promise(function (resolve, reject) {
        ...
        _overrideOdataModelValue: function (oParameter, oEvent) {
            var oModel = oEvent.getSource(),
                sUrl = oEvent.getParameter("url").replace(/\?.*/, ""),
                sFullPath = oParameter.editorMetadata.editorInfo.bindingPath + "/" + oParameter.editorMetadata.editorInfo.propertyName,
                sFullOdataUrl = oParameter.editorMetadata.editorInfo.odataURL + oParameter.editorMetadata.editorInfo.bindingPath;
            // check if the parameter has the same oData URL as the completed request
            if (sFullOdataUrl !== sUrl) { return; }
