    ...
    return Controller.extend("sap.ushell.components.pages.controller.ImportDialog", {
        ...
        _updateVisualization: function (pagesService, group, tileData, index, vizIndex, defaultGroupImported) {
            if (tileData.bUpdateNeeded) {
                ...
                return pagesService.updateVisualization(this.iPageIndex, iSectionIndex, vizIndex, tileData);
            }
            ...
            if (tile.isABookmark) {
                if (tile.isCustomBookmark) {
                    this.aPromiseChain.push(function () {
                        return services.bookmark.addCustomBookmark(tile.vizType, tile, contentNode)
                    ...
                    this.aPromiseChain.push(function () {
                        return services.bookmark.addBookmark(tile, contentNode)
                            .then(this._moveVisualization.bind(this, services.pages, group, index, vizIndex, defaultGroupImported))
                            .then(this._updateVisualization.bind(this, services.pages, group, tile, index, vizIndex, defaultGroupImported));
                    }.bind(this));
                }
        ...
        _addToPresetSection: function (group, index, services, contentNode, defaultGroupImported) {
            ...
            var iExistingTilesLength = oPresetSection.visualizations.filter(function (viz) { return viz.displayFormatHint !== "compact"; }).length;
            var iExistingLinksLength = oPresetSection.visualizations.filter(function (viz) { return viz.displayFormatHint === "compact"; }).length;
        ...
        _gatherVizDataObjectFromChipInstance: function (oChipInstance) {
            ...
            if (oVizData.isABookmark) {
                ...
                oBookmarkInfo = JSON.parse(JSON.parse(oChipInstance.configuration).tileConfiguration);
                oVizData.title = oBookmarkInfo.display_title_text;
                oVizData.url = oBookmarkInfo.navigation_target_url;
                oVizData.icon = oBookmarkInfo.display_icon_url;
                oVizData.info = oBookmarkInfo.display_info_text;
                oVizData.subtitle = oBookmarkInfo.display_subtitle_text;
                oVizData.serviceUrl = oBookmarkInfo.service_url;
                oVizData.serviceRefreshInterval = oBookmarkInfo.service_refresh_interval;
                oVizData.numberUnit = oBookmarkInfo.display_number_unit;
                oVizData.vizConfig = oChipInstance.configuration;
            }
            ...
            if (oVizData.isCustomBookmark) {
                ...
                if (oBookmarkInfo.TILE_PROPERTIES) {
                    try {
                        var oTileProperties = JSON.parse(oBookmarkInfo.TILE_PROPERTIES);
