...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.renderers.fiori2.Shell", {
            ...
            window.onbeforeunload = function () {
                if (sap.ushell.Container && sap.ushell.Container.getDirtyFlag()
                    ...
                    && !(Device.browser.name === Device.browser.BROWSER.INTERNET_EXPLORER
                        && AppLifeCycleAI.getCurrentApplication().container.getApplicationType() === sap.ushell.ApplicationType.NWBC.type)
                ) {
            ...
            return new Promise(function (fnResolve) {
                that._openAppViaNWBC(oResolvedHashFragment).then(function (bOpenedViaNWBC) {
                    if (bOpenedViaNWBC === true) {
                        ...
                        oDashboard = sap.ui.getCore().byId("sapUshellDashboardPage");
                        if (oDashboard) {
                            oDashboard.setBlocked(false);
                            oDashboard.setBusy(false);
                        }
            ...
            return new Promise(function (fnResolve) {
                ...
                oPromiseParams.then(
                    ...
                    function (oPostBodyParams) {
                        try {
                            sap.ui.getCore().getEventBus().publish("launchpad", "appOpening", oResolvedHashFragment);
                            if (!oEPCM.doNavigate(sUrlWithSapUserAndShellParam, iMode, undefined, undefined, undefined, undefined, undefined, oPostBodyParams)) {
                                fnResolve(false);
                                return;
                            }
                            sap.ui.getCore().getEventBus().publish("sap.ushell", "appOpened", oResolvedHashFragment);
        ...
        _toggleMenu: function (bVisible) {
            var oContainer = sap.ui.getCore().byId("menuBarComponentContainer");
         ...
         */
        _isDock: function () {
            return jQuery(".sapUshellContainerDocked").length !== 0;
        },
        ...
        _isDockingAreaOpen: function () {
            return jQuery(".sapUshellShellDisplayDockingAreaRight").length !== 0 || jQuery(".sapUshellShellDisplayDockingAreaLeft").length !== 0;
        },
        ...
        _updateUserPrefModel: function (entryObject) {
            var newEntry = this._getModelEntryFromEntryObject(entryObject),
                userPreferencesEntryArray = Config.last("/core/userPreferences/entries");
            ...
            Config.emit("/core/userPreferences/entries", userPreferencesEntryArray);
        },

        _updateProfilingModel: function (entryObject) {
            var newEntry = this._getModelEntryFromEntryObject(entryObject),
                userProfilingArray = Config.last("/core/userPreferences/profiling") || [];

            userProfilingArray.push(newEntry);
            Config.emit("/core/userPreferences/profiling", userProfilingArray);
        },
        ...
        _reorderUserPrefEntries: function (aEntries) {
            ...
            for (var i = aEntries.length - 1; i >= 0; i--) {
                if (aOrderedIds.indexOf(aEntries[i].id) !== -1) {
                    oSpecialEntry = aEntries.splice(i, 1)[0];
