...
sap.ui.define([
     ...
     *
     * @param {string[]} aUrlParams url parameters which should be added to url
     *
     * @returns {string} new query string
     * @private
     * @since 1.86.0
     */
    WindowUtils._getAdjustedQueryString = function (aUrlParams) {
     ...
     * @since 1.86.0
     */
    WindowUtils.refreshBrowser = function (aUrlParams) {
        var sNewHref = ushellUtils.getLocationHref(),
            sNewLocationSearch = WindowUtils._getAdjustedQueryString(aUrlParams);
        if (sNewLocationSearch) {
            sNewHref = sNewHref.replace(location.search, "?" + sNewLocationSearch);
        }
        if (ushellUtils.getLocationHref() === sNewHref) {
            window.location.reload(true /* bHardReload */);
        } else {
            window.location = sNewHref;
        }
    };
