...
sap.ui.define([
    ...
    TileRenderer.render = function (rm, tile) {
        ...
        if (oTileContainer && oTileContainer.getTiles) {
            oTiles = oTileContainer.getTiles();
        } else if (oTileContainer && oTileContainer.getAppBoxesContainer && oTileContainer.getCustomTilesContainer) {
            // AppFinder: CatalogEntryContainer
            oTiles = oTileContainer.getAppBoxesContainer().concat(oTileContainer.getCustomTilesContainer());
        }
        var oVisibleTiles = oTiles.filter(function (oTile) {
            return oTile.getVisible();
        });
        var iCurrentItemIndex = oVisibleTiles.indexOf(tile) > -1 ? oVisibleTiles.indexOf(tile) + 1 : "";
