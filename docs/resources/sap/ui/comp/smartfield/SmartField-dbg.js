...
sap.ui.define([
		...
		renderer: {
			...
			render: function(oRm, oControl) {
				var oContent = oControl.getContent(),
					oEditControl = oControl._oControl.edit,
				...
				if (oControl.getMode() === "edit" && !oControl._getFocusCatcher() &&
					(oEditControl === null || (oEditControl !== null && oContent !== null && oEditControl.getId() !== oContent.getId()))) {
					oControl._createFocusCatcher();
					oRm.renderControl(oControl._getFocusCatcher());
				} else if (oContent) {
					oRm.renderControl(oContent);

					if (oControl._isFocusCatcherFocused() &&
						oEditControl && oEditControl.getId() === oContent.getId()) {
						oContent.focus();
					}

					if (oControl._getFocusCatcher()) {
						oControl._destroyFocusCatcher();
					}
				}
				...
				var oDelegate = {
					onAfterRendering: function (oEvent) {
						...
						if (
							...
							// There is no focus catcher or it's not rendered
							(
								!oFocusCatcher ||
	...
	SmartField.prototype._createControlIfRequired = function(oSettings) {
		...
		if ((sMode !== this._oControl.current) || !vControlForCurrentMode) {
			...
			if (!vControlForCurrentMode) {
				...
				if (oControl) {
					if (sMode !== "edit" && this._getFocusCatcher()) {
	...
	SmartField.prototype._createFocusCatcher = function () {
		this.__oFakeInput = new sap.m.Input(this.getId() + "-fakeInput");
	};

	SmartField.prototype._destroyFocusCatcher = function () {
		if (this.__oFakeInput) {
			this.__oFakeInput.destroy();
			this.__oFakeInput = null;
		}
	};

	SmartField.prototype._getFocusCatcher = function () {
		return this.__oFakeInput;
	};

	SmartField.prototype._isFocusCatcherFocused = function () {
		return this._getFocusCatcher() && (this._getFocusCatcher().getId() + "-inner") === document.activeElement.getAttribute("id");
	};
	...
	SmartField.prototype.onAfterRendering = function fnOnAfterRenderingHandler() {
		...
		if (this.__oFakeInput) {
			setTimeout(function(){
				if (this.__oFakeInput) {
					this.__oFakeInput.setBlocked(true);
				}
			}.bind(this), 1000);
		}
	};
	...
	SmartField.prototype.onBeforeRendering = function fnOnBeforeRenderingHandler() {
		...
		if (this.__oFakeInput) {
			this.__oFakeInput.setBlocked(false);
		}
