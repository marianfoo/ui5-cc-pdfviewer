...
sap.ui.define([
	...
	HitTester.prototype.hitTest = function(x, y, width, height, renderer, scene, camera, clippingPlanes, options) {
		...
		if (object) {
			...
			// var mesh = new THREE.Mesh(new THREE.TetrahedronGeometry(0.003, 3), new THREE.MeshPhongMaterial({ color: 0x0080FF }));
			// mesh.position.copy(point);
			// mesh.scale.setScalar(new THREE.Vector4().copy(point).applyMatrix4(testCamera.matrixWorldInverse).applyMatrix4(testCamera.projectionMatrix).w);
			// mesh.updateMatrix();
		...
		if (intersects) {
			for (var i in intersects) {
				...
				if (object.visible && !object.isBillboard && !object.isDetailView) {
					...
					// var mesh = new THREE.Mesh(new THREE.TetrahedronGeometry(0.004, 3), new THREE.MeshPhongMaterial({ color: 0xFFFF00, opacity: 0.3, transparent: true  }));
					// mesh.position.copy(result.point);
					// mesh.scale.setScalar(new THREE.Vector4().copy(result.point).applyMatrix4(nativeCamera.matrixWorldInverse).applyMatrix4(nativeCamera.projectionMatrix).w);
					// mesh.updateMatrix();
					// nativeScene.children[0].add(mesh);
					return result;
				}
			}
		}
