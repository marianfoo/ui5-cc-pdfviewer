...
sap.ui.define([
	...
	Viewport.prototype._onVisibilityChanged = function(event) {
		function checkBackgroundNode(node) {
			return node.visible && node._vkGetNodeContentType() === NodeContentType.Background && bgNodeNameToProjectionType[node.name] !== undefined;
		}

		function findBackgroundNode(nodes) {
			var bgNode = nodes.find(checkBackgroundNode);
			for (var i = 0; !bgNode && i < nodes.length; i++) {
				bgNode = findBackgroundNode(nodes[i].children);
			}
			return bgNode;
		}
		...
		var bgNode = visibleNodes && findBackgroundNode(visibleNodes);
		if (bgNode != null) { // set background node
			this._backgroundNode = bgNode;
		} else if (this._backgroundNode != null) {
			var hiddenNodes = event.getParameter("hidden");
			if (this._backgroundNode === (hiddenNodes && hiddenNodes.find(checkBackgroundNode))) { // reset background node
	...
	Viewport.prototype._activateSingleView = function(view, playViewGroup, notAnimateCameraChange) {
		...
		if (camera) {
			...
			timeIntervalCameraChange = this._viewportGestureHandler.activateCamera(camera.getCameraRef(), notAnimateCameraChange ? 0 : timeInterval, pauseWhenNoCameraChange, transitionEffect);
		} else {
