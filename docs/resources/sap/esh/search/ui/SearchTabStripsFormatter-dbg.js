  ...
  var Tree = /*#__PURE__*/function () {
    ...
    _createClass(Tree, [{
      ...
      value: function updateFromPerspective(dataSource, perspective) {
        ...
        var currentCount = 0;
        var resultDataSources; // get count from search results (from ESH search without App search result count)

        if (perspective) {
          currentCount = perspective.totalCount;
        }

        var currentNode = this.findNode(dataSource); // node for dataSource already exists

        if (currentNode) {
          resultDataSources = this.collectDataSourcesFromResult(currentNode, perspective); // remove nodes if not needed for current tree
          ...
          this.removeObsoleteTreeNodes(this.rootNode, resultDataSources); // dataSource is "My Favorites", node is always below "All"
        ...
        currentNode.setCount(currentCount); // for root node: add apps count to count
        ...
        this.updateTreeFromPerspective(currentNode, resultDataSources); // update count for "My Favorites" node
        ...
        if (this.model.userCategoryManager && this.model.userCategoryManager.isFavActive()) {
          if (currentNode.dataSource === this.model.allDataSource) {
            ...
            if (!myFavNode) {
              ...
              }, 0), this); // filter all isFavDataSources from resultDataSources, from the result array sum all measureValues
      ...
      value: function updateCountMyFavorites(currentNode, resultDataSources) {
        ...
        if (myFavNode && currentNode.dataSource === this.model.allDataSource) {
          ...
          myFavNode.setCount(resultDataSources.filter(function (x) {
            ...
            return sum + cur.measureValue;
          }, 0)); // Apps are included in "My Favorites" and not included in resultDataSources
      ...
      value: function getFacets(currentNode, perspective) {
        ...
        var treeFacets = []; // no search results or dataSource is Apps
      ...
      value: function collectDataSourcesFromResult(currentNode, perspective) {
        ...
        var resultDataSource = {}; // Apps can only exists in All or "My Favorites"
        ...
        if ((currentNode.dataSource === this.model.allDataSource || currentNode.dataSource === this.model.favDataSource) && this.model.getProperty("/appCount") > 0) {
          ...
          resultDataSource.dimensionValueFormatted = this.model.appDataSource.labelPlural; // current dataSource is "My Favorites" and Apps is included in "My Favorites" or (Apps in MyFav)
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            if (resultNode) {
              ...
              childNode.unsureWhetherNodeisBelowRoot = false; // resultDataSource is included in "My Favorites" and must be inserted in the "My Favorites" node
