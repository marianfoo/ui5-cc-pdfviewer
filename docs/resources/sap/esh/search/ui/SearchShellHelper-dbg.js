  ...
  var SearchShellHelper = /*#__PURE__*/function () {
    ...
    _createClass(SearchShellHelper, null, [{
      ...
      value: function init() {
        ...
        this.isInitialized = true; // pre-fetch all app tiles

        sap.ushell.Container.getServiceAsync("Search").then(function (service) {
          service.prefetch();
        }); // get search model

        this.oModel = sap.esh.search.ui.getModelSingleton({}, "flp"); // create search field group control

        this.oSearchFieldGroup = new SearchFieldGroup("searchFieldInShell");
        this.oSearchFieldGroup.setModel(this.oModel); // initialize search input

        this.oSearchInput = this.oSearchFieldGroup.input;
        this.oSearchInput.setMaxSuggestionWidth("30rem");
        this.oSearchInput.setValue(this.oModel.getSearchBoxTerm()); // initialize search select
        ...
        this.oSearchFieldGroup.setCancelButtonActive(false); // add search field to shell header

        this.oShellHeader = sap.ui.getCore().byId("shell-header");
        this.oShellHeader.setSearch(this.oSearchFieldGroup); // create search field state manager
        ...
        jQuery(document).on("keydown", this.fnEscCallBack.bind(this)); // register for global events

        sap.ui.getCore().getEventBus().subscribe("shell", "searchCompLoaded", this.onSearchComponentLoaded.bind(this), {});
        this.oModel.subscribe("ESHSearchFinished", this.onAllSearchFinished.bind(this), {});
      ...
      value: function sizeSearchFieldChanged(event) {
        var size = event.getParameters()["remSize"]; // display mode of connector dropdown
