  ...
  var SearchFacetsFormatter = /*#__PURE__*/function () {
    ...
    _createClass(SearchFacetsFormatter, [{
      ...
      value: function getAttributeFacetsFromPerspective(resultSet, searchModel) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            for (var j = 0; j < oServerSideFacet.items.length; j++) {
              var oFacetListItem = oServerSideFacet.items[j]; // DWC exit, add icon only for space facet
        ...
        this.addDataTypeToClientSideFacets(aClientSideFacets, searchModel); // create facet items from global filter

        var oClientSideFacetsWithSelection = {};

        var aFacetItemsWithFilterConditions = this._createFacetItemsFromConditionGroup(oDataSource, searchModel.getProperty("/uiFilter/rootCondition")); // combine facets from global filter with facets from server
          ...
          this.treeQuickSelectDataSourceFacet = {
            ...
            items: [{
              ...
              children: searchModel.config.quickSelectDataSources.map(function (treeNodeProps) {
                return _this2.createTreeNodeQuickSelectDataSource(treeNodeProps);
              })
            }]
          };
        }
      ...
      value: function expandPathToSelectedDataSource(searchModel, rootNode) {
        ...
        var paths = collectPaths(rootNode, searchModel.getDataSource()); // expand paths
      ...
      value: function createTreeNodeQuickSelectDataSource(treeNodeProps) {
        ...
        if (treeNodeProps.children) {
          children = treeNodeProps.children.map(function (childTreeNodeProps) {
            return _this3.createTreeNodeQuickSelectDataSource(childTreeNodeProps);
          });
        }
      ...
      value: function getFacets(oDataSource, oINAPerspective, searchModel) {
        ...
        return _call(function () {
          ...
          var resultFacets = [_this.getDataSourceFacetFromTree(searchModel)]; // add quick datasource select facet

          _this.addQuickSelectDataSourceFacet(searchModel, resultFacets); // for ds=apps or ds=category -> no attribute facets, just return
          ...
          var collectedFacets = []; // add dynamic hierarchy facets

          return _await(_this.hierarchyDynamicFacetsFormatter.getFacets(oINAPerspective, searchModel), function (hierarchyDynamicFacets) {
            collectedFacets.push.apply(collectedFacets, _toConsumableArray(hierarchyDynamicFacets)); // add static hierarchy facets

            return _await(_this.hierarchyStaticFacetsFormatter.getFacets(oINAPerspective), function (hierarchyStaticFacets) {
              collectedFacets.push.apply(collectedFacets, _toConsumableArray(hierarchyStaticFacets)); // add attribute facets

              return _await(_this.getAttributeFacetsFromPerspective(oINAPerspective, searchModel), function (attributeFacets) {
                collectedFacets.push.apply(collectedFacets, _toConsumableArray(attributeFacets)); // sort

                _this.sortFacets(collectedFacets, searchModel);

                resultFacets.push.apply(resultFacets, collectedFacets);

                _this.setFacetIndex(resultFacets);
      ...
      value: function getAttributeDialogFacetsFromMetaData(oMetaData, searchFacetDialogModel) {
        ...
        var aClientSideFacets = []; // extract facets from server response:
      ...
      value: function hasDialogFacetsFromMetaData(searchModel) {
        ...
        var hasDialogFacets = false; // extract facets from server response:
