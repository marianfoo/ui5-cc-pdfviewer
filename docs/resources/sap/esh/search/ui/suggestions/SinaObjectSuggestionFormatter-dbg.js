...
(function(){
sap.ui.define(["sap/esh/search/ui/SearchNavigationObjectForSinaNavTarget"], function (SearchNavigationObjectForSinaNavTarget) {
  ...
  var Formatter = /*#__PURE__*/function () {
    ...
    _createClass(Formatter, [{
      ...
      value: function format(suggestionProvider, sinaSuggestion) {
        // create suggestion
        var suggestion = sinaSuggestion; // assemble label

        var label1 = this.assembleLabel1(sinaSuggestion);
        suggestion.label1 = label1.label; // assemble second label (second line in UI)

        suggestion.label2 = this.assembleLabel2(label1.isHighlighted, sinaSuggestion); // assemble navigation target

        suggestion.titleNavigation = this.assembleNavigation(sinaSuggestion, suggestionProvider.model); // assemble image url

        var imageUrl = this.assembleImageUrl(sinaSuggestion);
        sinaSuggestion.sina.core.extend(suggestion, imageUrl); // position

        suggestion.position = sinaSuggestion.position; // add
