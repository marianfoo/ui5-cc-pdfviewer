...
(function(){
sap.ui.define(["../i18n", "./SinaBaseSuggestionProvider", "./SinaObjectSuggestionFormatter", "./SuggestionType", "sap/esh/search/ui/SearchHelper"], function (__i18n, __SinaBaseSuggestionProvider, __SinaObjectSuggestionFormatter, ___SuggestionType, SearchHelper) {
  ...
  var SinaSuggestionProvider = /*#__PURE__*/function (_SinaBaseSuggestionPr) {
    ...
    function SinaSuggestionProvider(options) {
      ...
      _this2 = _super.call(this, options.sinaNext);
      _this2.model = options.model;
      _this2.suggestionTypes = options.suggestionTypes;
      _this2.suggestionHandler = options.suggestionHandler;
      _this2.suggestionLimit = sap.ui.Device.system.phone ? 5 : 7;
      _this2.suggestionStartingCharacters = _this2.model.config.suggestionStartingCharacters; // this.dataSourceDeferred = null;

      _this2.sinaObjectSuggestionFormatter = new SinaObjectSuggestionFormatter();
      return _this2;
    } // abort suggestions
    // ===================================================================
    ...
    _createClass(SinaSuggestionProvider, [{
      ...
      value: function getSuggestions(filter) {
        ...
        return _call(function () {
          // reset global fields
          _this.suggestions = [];
          _this.firstObjectDataSuggestion = true;
          _this.numberSuggestionsByType = {};

          for (var i = 0; i < UISuggestionTypeProperties.types.length; ++i) {
            var suggestionType = UISuggestionTypeProperties.types[i];
            _this.numberSuggestionsByType[suggestionType] = 0;
          } // data based search term suggestions only starting by default from 3. character


          var suggestionTerm = filter.searchTerm;

          if (_this.suggestionTypes.length === 1 && _this.suggestionTypes.indexOf(UISuggestionType.SearchTermData) >= 0 && suggestionTerm.length < _this.suggestionStartingCharacters) {
            return Promise.resolve(_this.suggestions);
          } // object suggestions only starting by default from 3. character


          if (_this.suggestionTypes.length === 1 && _this.suggestionTypes.indexOf(UISuggestionType.Object) >= 0 && suggestionTerm.length < _this.suggestionStartingCharacters) {
            return Promise.resolve(_this.suggestions);
          } // data source suggestions only for ds=all


          if (_this.suggestionTypes.length === 1 && _this.suggestionTypes.indexOf(UISuggestionType.DataSource) >= 0 && _this.model.getDataSource() !== _this.model.sinaNext.allDataSource) {
            return Promise.resolve(_this.suggestions);
          } // handle client side datasource-suggestions for all and apps


          _this.createAllAndAppDsSuggestions(); // check that BO search is enabled
          ...
          if (_this.model.getDataSource() === _this.model.appDataSource) {
            return Promise.resolve(_this.suggestions);
          } // prepare sina suggestion query


          _this.prepareSuggestionQuery(filter); // fire sina suggestion query


          return _await(_this.suggestionQuery.getResultSetAsync(), function (resultSet) {
            // concatenate searchterm + suggestion term
            var sinaSuggestions = resultSet.items; // assemble items from result set

            _this.formatSinaSuggestions(sinaSuggestions);
      ...
      value: function preFormatSuggestions(sinaSuggestions) {
        for (var i = 0; i < sinaSuggestions.length; ++i) {
          ...
          var uiSuggestion = sinaSuggestion; // suggestion type

          uiSuggestion.uiSuggestionType = this.getSuggestionType(sinaSuggestion); // set position

          uiSuggestion.position = UISuggestionTypeProperties.properties[uiSuggestion.uiSuggestionType].position; // key

          this.assembleKey(uiSuggestion); // process children
          ...
          case UISuggestionType.Object:
            {
              ...
              var objKey = sinaSuggestion.object.detailAttributes[0].value;
      ...
      value: function formatSinaSuggestions(sinaSuggestions) {
        ...
        this.preFormatSuggestions(sinaSuggestions); // process suggestions

        for (var i = 0; i < sinaSuggestions.length; ++i) {
          var sinaSuggestion = sinaSuggestions[i]; // limit number of suggestions
