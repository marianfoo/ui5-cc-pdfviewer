...
sap.ui.define(["sap/m/MessageBox", "sap/ui/core/format/FileSizeFormat", "sap/ui/core/format/NumberFormat", "./i18n", "./UIUtil"], function (MessageBox, FileSizeFormat, NumberFormat, __i18n, ___UIUtil) {
  ...
  var Tester = /*#__PURE__*/function () {
    // CSS class
    function Tester(sSearchTerms, highlightStyle, // CSS class
    matchContains, bDisjunctionType) {
      ...
      sSearchTerms = sSearchTerms || "*"; // asterisk
      ...
      sSearchTerms = sSearchTerms.replace(/([.^=!:${}()|\+\-\[\]\/\\])/g, "\\$1"); // store tokenized search terms array
      ...
      for (var j = 0; j < this.aSearchTerms.length; ++j) {
        ...
        searchTerm = searchTerm.replace(/\*/g, ".*?"); // check if search term is Chinese (in unicode Chinese characters interval).
    ...
    _createClass(Tester, [{
      ...
      value: function test(sText2BeSearched) {
        ...
        this.initializeBoldArray(sText2BeSearched); // global flag is there is any bold char
        ...
        for (var j = 0; j < this.aSearchTerms.length; ++j) {
          // only match beginnings of the word, case insensitive
          oRegSearchTerm = this.aSearchTerms[j]; // match?

          var lastIndex = -1;
          var bLocalMatch = false;

          while ((aMatchResult = oRegSearchTerm.exec(sText2BeSearched)) !== null) {
            bLocalMatch = true; // prevent endless loop, should not happen but who knows...

            if (oRegSearchTerm.lastIndex === lastIndex) {
              break;
            }

            lastIndex = oRegSearchTerm.lastIndex; //aMatchResult.index: the start position of matching term
        ...
        oReturn.bMatch = true; // generate highligted text
      ...
      value: function tokenizeSearchTerms(terms) {
        ...
        var newTerms = []; // split search terms with space and wildcard into array
  ...
  function delayedExecution( // eslint-disable-next-line @typescript-eslint/ban-types
  ...
  function refuseOutdatedRequests(originalFunction, requestGroupId) {
    ...
    var decorator = function decorator() {
      ...
      var requestId = ++lastRequestId; //console.log(requestGroupId + ' start ', requestId);
    ...
    decorator.abort = function () {
      ++lastRequestId; // console.log(id + ' abort', maxRequestId);
    };
  ...
  function boldTagUnescaperByText(domref) {
    var $d = $(domref); // Security check, whether $d.text() contains tags other than <b> and </b>
  ...
  var hasher = {
    ...
    showUrlUpdateError: function showUrlUpdateError() {
      ...
      this.urlError = true; // show message box
  ...
  function subscribeOnlyOnce(id, eventName, // eslint-disable-next-line @typescript-eslint/ban-types
  callBack, // eslint-disable-next-line @typescript-eslint/ban-types
  selfControl) {
    if (idMap[id]) {
  ...
  var SearchFocusHandler = /*#__PURE__*/function () {
    ...
    _createClass(SearchFocusHandler, [{
      ...
      value: function get2BeFocusedControlDomRef() {
        ...
        var isGridDisplayed = false; // let isMapDisplayed = false;
        ...
        if (this.oModel.getDataSource() !== this.oModel.appDataSource) {
          ...
          if (this.oModel.getProperty("/boCount") > 0 && this.oModel.getProperty("/appCount") > 0) {
            ...
            control = this.oSearchComposite["searchResultList"].getItems()[index]; // ToDo, adjust ["searchResultList"] as soon as SearchCompositeControl js->ts done, also for appSearchResult -> multiple occ in this file
            ...
            isGridDisplayed = this.oModel.getResultViewType() === "searchResultGrid"; // isMapDisplayed = this.oModel.getResultViewType() === "searchResultMap";
            ...
            // 1.2.4 map view
            //     control = this.oSearchComposite.searchResultMap;
            // }
          ...
          index = this.oModel.getProperty("/focusIndex");
          oTilesContainer["_oItemNavigation"].setFocusedIndex(index); // oTilesContainer dosen't have tiles aggregation
      ...
      value: function setFocus() {
        ...
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        var that = this;
        var retries = 20; // method for setting the focus with periodic retry

        var doSetFocus = function doSetFocus() {
          that.focusSetter = null;
          var controlDomRef = that.get2BeFocusedControlDomRef(); // this is a workaround to prevent first app tile and input share the same sapenter event
          ...
          controlDomRef.focus(); //console.log('--set');
  ...
  function attachEventHandlersForTooltip(element) {
    ...
    registerHandler("tooltip-mouseenter", $element.find(".sapUshellSearchResultListItem-MightOverflow"), "mouseenter", function () {
      ...
      if (!$this.attr("title") && (this.offsetWidth < this.scrollWidth || // && this.getBoundingClientRect().width == this.offsetWidth ||
