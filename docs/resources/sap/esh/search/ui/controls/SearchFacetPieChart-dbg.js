...
sap.ui.define(["../i18n", "sap/ui/thirdparty/d3", "sap/ui/core/Control"], function (__i18n, thirdpartyD3, Control) {
  ...
  var SearchFacetPieChart = Control.extend("sap.esh.search.ui.controls.SearchFacetPieChart", {
    renderer: {
      ...
      render: function render(oRm, oControl) {
        ...
        oRm.openEnd(); // close SearchFacetPieChart div
    ...
    getDataForPieChart: function _getDataForPieChart(data, model, facetIndex) {
      ...
      var pieChartTotal = 0;
      this.iMissingCnt = 0; // const sumSelected = this.getSumSelected(data);
          ...
          for (var j = 0; j < data[i].items.length; j++) {
            ...
            if (itemValue) {
              ...
              */
              ...
              item.removed = false;
              item.fill = "#007CAA";
              item.maxLabelLength = 30; //TO DO consider if only show top 5 how will user see there is a filter in place? on list view of facet the filter condition is listed sso leave it here too for now
    ...
    directUpdate: function _directUpdate(data, piechartParent, model, options) {
      ...
      if (!model1) {
        model1 = this.getParent().getModel(); // model1 = this.oParent.getModel();
      }
    ...
    onAfterRendering: function _onAfterRendering() {
      ...
      if (typeof SVGElement.prototype.blur === "undefined") {
        SVGElement.prototype.blur = function () {// for IE 11 initial error on clicking SVG
        };
      }
      ...
      if (!model) {
        model = this.getParent().getModel("facets"); // model = this.getParent().oModels.facets;
      }

      if (model) {
        piechartParent = $("#" + this["sId"])[0]; // ensure that we are in the 'small bar chart'

        if ($(piechartParent).parent()[0].className === "sapMLIBContent") {
          settings.pieChartParentClass = "sapUshellSearchFacetPieChart";
          facetIndex = this.getFacetIndexById(this["sId"]); // const oChart = sap.ui.getCore().byId(this.sId);

          piechartParent.className = settings.pieChartParentClass; // piechartIndex = this.getPieChartIndexByFacetIndex(facetIndex);
          ...
          chart.update(chartElements); // update infozeile
  ...
  var PieChart = /*#__PURE__*/function () {
    ...
    _createClass(PieChart, [{
      ...
      value: function init(parentSelector, settings, application, model) {
        ...
        var r = Math.min(svgWidth, svgHeight) / 2; // initialize this.options
        ...
        this.createAttributeService(PieChart, "arcCalculator"); // create global svg elements: parent.append

        var xOrigin = Math.round(this.options.width / 2);
        var yOrigin = Math.round(this.options.height / 2);
        this.svg = parent.append("svg:svg").attr("width", svgWidth).attr("height", svgHeight) // .attr("tabindex", "0")
        ...
        this.svg.attr("class", this.options.pieChartClass); // initialize svg size and tween functions

        this.inittween();
        this.oldArcs = []; // stack of clicked segments

        this.clickedSegment = {}; // mark init state as firstUpdate
      ...
      value: // create attribute get and set services
      // ======================================================================
      ...
      value: function inittween() {
        ...
        that.yOrigin = Math.round(that.options.height / 2); // adjust size

        that.svg.attr("width", this.options.width).attr("height", this.options.height); // tween function generator: generates tween function for svg:path
      ...
      value: function notAnimatedUpdate(data) {
        ...
        this.chartElements = data; // remove old elements
        ...
        var sel = this.svgArcs.selectAll("path").data(arcs, PieChart.getIdOfArc); // remove old arcs
        ...
        if (!sel.empty()) {
          sel.attr("d", function (d) {
            ...
            var translateStr; // translate all segments that.options.padding4arcs in outer direction
        ...
        .on("keydown", function () {
          ...
          if (labelElem) {
            ...
            labelposition.update(); // in case that label shall be cut
        ...
        sel = that.svgLabels.selectAll("g").data(arcs, PieChart.getIdOfArc); // remove
        ...
        if (!sel.empty()) {
          sel.style("opacity", function (d) {
            ...
            if (this.childNodes[1] && this.childNodes[1].childNodes[0]) {
              labelposition = new Labelposition(that.options.width, that.options.height, d.startAngle, d.endAngle, that.options.outerRadius, this.getBoundingClientRect().width, // was 'this' before
              this.getBoundingClientRect().height, // was 'this' before
              that.svgArcGen(d).centroid(d), d.data.label, that.svg); // log.debug("non animation change/remove existing labels, text exists: ", this.childNodes[1].childNodes[0].data);
            } else {
              labelposition = new Labelposition(that.options.width, that.options.height, d.startAngle, d.endAngle, that.options.outerRadius, this.getBoundingClientRect().width, // was 'this' before
              this.getBoundingClientRect().height, // was 'this' before
              that.svgArcGen(d).centroid(d), " ", that.svg); //                      //log.debug("non animation change/remove existing labels, no existing text ");
            }
        ...
        if (!sel.enter().empty()) {
          ...
          textsG.style("opacity", function (d) {
            ...
            var translateStr; // log.debug("non animation new labels, text exists: ", this.childNodes[1].childNodes[0].data);

            var labelposition = new Labelposition(that.options.width, that.options.height, d.startAngle, d.endAngle, that.options.outerRadius, this.getBoundingClientRect().width, // was 'this' before
            this.getBoundingClientRect().height, // was 'this' before
      ...
      value: function adjustLabelWidth(element, targetWidth) {
        ...
        var textElement = element.childNodes[1]; // just remove all children when targetWidth is 0

        if (targetWidth <= 0) {
          shadowElement.childNodes[0].data = "";
          textElement.childNodes[0].data = ""; // while (element.firstChild){
        ...
        textElement.childNodes[0].data = shadowElement.childNodes[0].data; // log.debug("label svg width:", shadowElement.getBBox().width, " label text: ", shadowElement.childNodes[0].data, " targetWidth: ", targetWidth, " original text: ", originalText, " originBoundingWidth: ", originBoundingWidth);
      } // get arcs with label
      // ======================================================================
      ...
      key: "insertAfter",
      value: // helper: insert zero sized arc after insertIndex into arcs
      // ======================================================================
      ...
      value: // generator: default arc calculation
      // ======================================================================
      ...
      value: function calculateNewArcsOnly(data) {
        ...
        // sort data
        data = data.slice(); // ie sort has reverse order of parameter comparing to FF and Chrome
      ...
      value: function calculateArcs(oldArcs, data) {
        var newArcs = PieChart.arcsGen()(data); // insert new elements with size 0 (startAngle=endAngle) into old arcs

        this.insertMissingArcs(oldArcs, newArcs, true); // insert removed elements with size 0 (startAngle=endAngle) into new arcs
      ...
      value: function determineInsertIndex(arcs1, arc1InsertIndex, arcs2, arc2Index) {
        ...
        var index; // determine index

        for (index = arc1InsertIndex + 1; index < arcs1.length; ++index) {
          var sequenceIndex2 = this.arcSequence.getIndex(arcs1[index].data.id); // exit condition based on historial information
      ...
      key: "getIndexById",
      value: // helper: get index of arc with given id in arcs
      // ======================================================================
      ...
      value: function translateStr4Padding(startAngle, endAngle, padding, xOldOrig, yOldOrig) {
        ...
        var yNewOrig = -(padding * Math.cos(apexAngle / 2)); // rotate it by startAngle
  ...
  _defineProperty(PieChart, "Tweens", function () {
    ...
    Tweens1.tweenGenSimple = function (d, i, a, innerRadius, outerRadius, svgGenerator) {
      ...
      var j = SearchFacetPieChart.d3.interpolate(s1, s2); // svgGenerator = svgGenerator;
    ...
    Tweens1.tweenGenSimpleText = function (d, i, a, innerRadius, outerRadius, chartInstance, filtered) {
      ...
      labelpositionAfter.update(); // in case that label shall be cut
    ...
    Tweens1.tweenGenEcstasy = function (d, i, a, innerRadius, outerRadius, svgGenerator) {
      ...
      var splitInterpolation = SearchFacetPieChart.d3.interpolate(s1, s2); // move + resize
      ...
      var resizeInterpolation = SearchFacetPieChart.d3.interpolate(s1, s2); // unify
      ...
      var unifyInterpolation = SearchFacetPieChart.d3.interpolate(s1, s2); // assemble total animation function
    ...
    Tweens1.tweenGenLSD = function (d, i, a, innerRadius, outerRadius, svgGenerator) {
      ...
      var splitInterpolation = SearchFacetPieChart.d3.interpolate(s1, s2); // move
      ...
      var moveInterpolation = SearchFacetPieChart.d3.interpolate(s1, s2); // resize
      ...
      var resizeInterpolation = SearchFacetPieChart.d3.interpolate(s1, s2); // unfiy
  ...
  var Labelposition = /*#__PURE__*/function () {
    ...
    function Labelposition(backgroundWidth, backgroundHeight, startAngle, endAngle, r, labelWidth, labelHeight, d3centroid, text, svgBackground) {
      ...
      var yOrigin = Math.round(this.backgroundHeight / 2); // translation to center
      ...
      value: function update() {
        ...
        do {
          ...
          if (optimizationDirection === 0) {
            ...
            c2 = this.calc(outset - this.outsetStep, this.labelWidth); /// value solutions

            c1v = this.value(c1, c);
            c2v = this.value(c2, c); //log.debug("           current ", " labelShortened", c.labelShortened, "labelWidth", c.labelWidth, "xSpace", c.xSpace, "asymmetry", c.asymmetry, " x:", c.centroidX, " y:", c.centroidY);
            ...
            if (c1v < c2v && c2v > 0.5) {
              ...
              optimizationDirection = -1; // log.debug("           -> going inwards");
              ...
              optimizationDirection = 1; // log.debug("           -> going outwards");
            ...
            c1v = this.value(c1, c); // log.debug("           current ", " labelShortened", c.labelShortened, "labelWidth", c.labelWidth, "xSpace", c.xSpace, "asymmetry", c.asymmetry, " x:", c.centroidX, " y:", c.centroidY);
            // log.debug("           outset+-", "labelShortened", c1.labelShortened, "labelWidth", c1.labelWidth, "xSpace", c1.xSpace, "asymmetry", c1.asymmetry, " value=>", c1v, " x:", c1.centroidX, " y:", c.centroidY);
        ...
        this.x = isNaN(x) ? 0 : x;
        this.y = isNaN(x) ? 0 : y; //log.debug("=======final x:" + this.x + " y:" + this.y + " labelwidth:" + this.labelWidth);
      }
    }, {
      key: "calc",
      value: function calc(outset, labelWidth) {
        var centroidX, centroidY, leftBorder, rightBorder, perimeterRightBorder, perimeterLeftBorder; /// full circle?
        ...
        if (this.startAngle - this.endAngle + 2 * Math.PI < 1e-9) {
          ...
          perimeterRightBorder = this.r; //log.debug("full circle -> placement at 0,0 regardless of outset");
        } else {
          // Calculate Centroid
          ...
          centroidX = tmp; //log.debug("======Schwerpunkt x:" + centroidX + " y:" + centroidY);
        ...
        labelWidth = Math.min(labelWidth, Math.floor(rightBorder - leftBorder)); // calculate x
        ...
        var y = centroidY; // return coordinates and parameters of the placement
  ...
  var HistoricalArcCalculator = /*#__PURE__*/function () {
    ...
    _createClass(HistoricalArcCalculator, [{
      ...
      value: function calculateArcs(oldArcs, data) {
        ...
        this.insertMissingArcs(oldArcs, newArcs); // insert removed elements with size 0 (startAngle=endAngle) into new arcs
