...
sap.ui.define(["../core/core", "../core/util", "./SinaObject", "./Filter", "./LogicalOperator", "../core/errors", "./FilteredDataSource"], function (core, util, ___SinaObject, ___Filter, ___LogicalOperator, ___core_errors, ___FilteredDataSource) {
  ...
  var Query = /*#__PURE__*/function (_SinaObject) {
    ...
    function Query(properties) {
      ...
      _defineProperty(_assertThisInitialized(_this2), "requestTimeout", false);

      _this2.top = (_properties$top = properties.top) !== null && _properties$top !== void 0 ? _properties$top : 10;
      _this2.skip = (_properties$skip = properties.skip) !== null && _properties$skip !== void 0 ? _properties$skip : 0;
      _this2.sortOrder = (_properties$sortOrder = properties.sortOrder) !== null && _properties$sortOrder !== void 0 ? _properties$sortOrder : [];
      _this2.filter = (_ref = (_properties$filter = properties.filter) !== null && _properties$filter !== void 0 ? _properties$filter : _this2.filter) !== null && _ref !== void 0 ? _ref : new Filter({
        sina: _this2.sina
      });
      _this2.icon = properties.icon;
      _this2.label = properties.label;

      if (properties.dataSource) {
        _this2.filter.setDataSource(properties.dataSource);
      }

      if (properties.searchTerm) {
        _this2.filter.setSearchTerm(properties.searchTerm);
      }

      if (properties.rootCondition) {
        _this2.filter.setRootCondition(properties.rootCondition);
      }

      if (_this2.requestTimeout) {// this._execute = util.timeoutDecorator(this._execute, this.requestTimeout);
      }

      _this2._execute = util.refuseOutdatedResponsesDecorator(_this2._execute);
      return _this2;
    }
    ...
    _createClass(Query, [{
      ...
      key: "abort",
      value: function abort() {// TODO: Promise has no abort
      ...
      value: function getResultSetAsync() {
        ...
        return _call(function () {
          if (_this._lastQuery) {
            ...
            if (_this.equals(_this._lastQuery // EqualsMode.CheckFireQuery
            )) {
              return _await(_this._resultSetPromise);
            } // filter changed -> set skip=0


            if (!_this.filter.equals(_this._lastQuery.filter)) {
              _this.setSkip(0);
            }
          } // create a read only clone


          _this._lastQuery = _this._createReadOnlyClone(); // delegate to subclass implementation

          var resultSet;
          _this._resultSetPromise = Promise.resolve().then(function () {
            return this._execute(this._lastQuery);
          }.bind(_this)).then(function (iResultSet) {
