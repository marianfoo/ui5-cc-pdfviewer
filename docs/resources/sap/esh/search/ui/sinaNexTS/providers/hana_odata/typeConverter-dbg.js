...
sap.ui.define(["../../sina/util", "../../sina/AttributeType", "../../core/errors"], function (sinaUtil, ____sina_AttributeType, ____core_errors) {
  ...
  function sina2OdataTimestamp(value) {
    ...
    var result = this.addLeadingZeros(year.toString(), 4) + "-" + this.addLeadingZeros(month.toString(), 2) + "-" + this.addLeadingZeros(day.toString(), 2) + "T" + this.addLeadingZeros(hour.toString(), 2) + ":" + this.addLeadingZeros(minute.toString(), 2) + ":" + this.addLeadingZeros(seconds.toString(), 2) + "Z"; // this.addLeadingZeros(microseconds.toString(), 7) + 'Z';
  ...
  function odata2SinaTime(value) {
    ...
    // sina: hh:mm:ss
    value = value.trim();
    return value; //            if(value.indexOf(":") === 2){
    //                return value;
    //            }else{
  ...
  function sina2OdataTime(value) {
    // odata: hh:mm:ss
    // sina: hh:mm:ss
    return value; //            return value.slice(0, 2) + value.slice(3, 5) + value.slice(6, 8);
  }
  ...
  function odata2SinaDate(value) {
    ...
    // sina: YYYY/MM/DD
    value = value.trim();
    return value.slice(0, 4) + "/" + value.slice(5, 7) + "/" + value.slice(8, 10); //return value.slice(0, 4) + '/' + value.slice(4, 6) + '/' + value.slice(6, 8);
  }

  function sina2OdataDate(value) {
    // odata: YYYY-MM-DD
    // sina: YYYY/MM/DD
    return value.slice(0, 4) + "-" + value.slice(5, 7) + "-" + value.slice(8, 10); //return value.slice(0, 4) + value.slice(5, 7) + value.slice(8, 10);
  }
