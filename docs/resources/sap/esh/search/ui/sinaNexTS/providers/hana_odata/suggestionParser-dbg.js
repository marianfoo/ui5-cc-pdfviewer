  ...
  var SuggestionParser = /*#__PURE__*/function () {
    ...
    _createClass(SuggestionParser, [{
      ...
      value: function parse(query, data) {
        var suggestions = [];
        var suggestion; // var parentSuggestion;
        // var parentSuggestions = [];
        // var suggestionSearchTerms = [];

        var cell; // var parentCell;
        // var calculationMode;

        for (var i = 0; i < data.length; i++) {
          suggestion = null;
          cell = data[i]; // calculationMode = this.parseCalculationMode(cell.Type);
          //
          // switch (cell.Type) {
          // case 'H':
          ...
          suggestion = this.parseSearchTermSuggestion(query, cell); // if (suggestion) {
      ...
      value: function parseObjectSuggestions(query, searchItems) {
        ...
        var objectSuggestions = []; // var objectSuggestions = searchItems.d.ObjectSuggestions.SearchResults.results;

        for (var i = 0; i < searchItems.length; ++i) {
          var object = searchItems[i]; // fill highlighted value: actually it would be better to call
