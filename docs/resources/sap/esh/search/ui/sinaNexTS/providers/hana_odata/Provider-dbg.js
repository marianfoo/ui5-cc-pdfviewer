...
sap.ui.define(["../../core/core", "../../core/ajax", "./MetadataParserXML", "./MetadataParserJson", "./ItemParser", "./FacetParser", "./suggestionParser", "./eshObjects/src/index", "./conditionSerializerEshObj", "../../core/Log", "../../sina/SearchQuery", "../../sina/SortOrder", "../AbstractProvider", "../../sina/SuggestionType", "../../sina/ComplexCondition", "../../core/errors", "./HierarchyParser", "./HierarchyNodePathParser", "../../sina/SuggestionCalculationMode"], function (core, ____core_ajax, ___MetadataParserXML, ___MetadataParserJson, ___ItemParser, ___FacetParser, ___suggestionParser, ___eshObjects_src_index, conditionSerializer, ____core_Log, ____sina_SearchQuery, ____sina_SortOrder, ___AbstractProvider, ____sina_SuggestionType, ____sina_ComplexCondition, ____core_errors, ___HierarchyParser, ___HierarchyNodePathParser, ____sina_SuggestionCalculationMode) {
  ...
  var Provider = /*#__PURE__*/function (_AbstractProvider) {
    ...
    function Provider() {
      ...
      _this8 = _super.call.apply(_super, [this].concat(args));

      _defineProperty(_assertThisInitialized(_this8), "id", "hana_odata");
    ...
    _createClass(Provider, [{
      ...
      value: function initAsync(configuration) {
        ...
        return _call(function () {
          ...
          _this.requestPrefix = configuration.url;
          _this.odataVersion = configuration.odataVersion;
          _this.responseAttributes = configuration === null || configuration === void 0 ? void 0 : configuration.responseAttributes;
          _this.facetAttributes = configuration === null || configuration === void 0 ? void 0 : configuration.facetAttributes;
          _this.sina = configuration.sina;
          _this.querySuffix = _this.convertQuerySuffixToExpression(configuration.querySuffix);
          _this.metaDataSuffix = (_configuration$metaDa = configuration.metaDataSuffix) !== null && _configuration$metaDa !== void 0 ? _configuration$metaDa : "";
          ...
          _this.ajaxClient = (_configuration$ajaxCl = configuration.ajaxClient) !== null && _configuration$ajaxCl !== void 0 ? _configuration$ajaxCl : new Client(clientProperties);
          var metaDataJsonType = configuration.metaDataJsonType;

          if (metaDataJsonType) {
            _this.metadataParser = new MetadataParserJson(_this);
          } else {
            _this.metadataParser = new MetadataParserXML(_this);
          }

          _this.itemParser = new ItemParser(_this);
          _this.facetParser = new FacetParser(_this);
          _this.suggestionParser = new SuggestionParser(_this);
          _this.hierarchyNodePathParser = new HierarchyNodePathParser(_this.sina);
          return _await(_this.loadServerInfo(), function (_this$loadServerInfo) {
            _this.serverInfo = _this$loadServerInfo;

            if (!_this.supports("Search")) {
              throw new ESHNotActiveError();
            }

            return _await(_this.loadBusinessObjectDataSources(), function () {
              return _this.sina.dataSources.length === 0 ? Promise.reject(new ESHNotActiveError("Enterprise Search is not active - no datasources")) : {
                capabilities: _this.sina._createCapabilities({
                  fuzzy: false
                })
              };
            });
      ...
      value: function loadBusinessObjectDataSources() {
        ...
        return _call(function () {
          var requestUrl = _this2._prepareMetadataRequest();

          return _await(_this2.metadataParser.fireRequest(_this2.ajaxClient, requestUrl), function (response) {
            return _await(_this2.metadataParser.parseResponse(response), function (allMetaDataMap) {
              for (var i = 0; i < allMetaDataMap.dataSourcesList.length; ++i) {
                var dataSource = allMetaDataMap.dataSourcesList[i];

                _this2.metadataParser.fillMetadataBuffer(dataSource, allMetaDataMap.businessObjectMap[dataSource.id]);
              }
            });
          });
        });
      ...
      value: function _prepareSearchObjectSuggestionRequest(query) {
        ...
        var searchTerms = query.filter.searchTerm || "*"; // DWC Specific logic - query suffix should not be added to helper hierarchy datasource.
      ...
      value: function assembleEshSearchQuery(searchOptions) {
        ...
        if (!((_this$sina = this.sina) !== null && _this$sina !== void 0 && (_this$sina$configurat = _this$sina.configuration) !== null && _this$sina$configurat !== void 0 && _this$sina$configurat.enableQueryLanguage) || !searchOptions.freeStyleText) {
      ...
      value: function _fireSearchQuery(oInputData) {
        ...
        return _call(function () {
          // fire request
          return _await(_this3.ajaxClient.getJson(oInputData.url), function (response) {
            _this3.metadataParser.parseDynamicMetadata(response);

            var hierarchyNodePaths = _this3.hierarchyNodePathParser.parse(response, oInputData.query);

            return _await(_this3.itemParser.parse(oInputData.query, response.data), function (items) {
              ...
              return _invoke(function () {
                if (oInputData.query.getDataSource() === _this3.sina.getAllDataSource() && statistics && Array.isArray(statistics) && statistics.length === 1) {
                  ...
                  return _await(_this3.facetParser.parse(oInputData.query, constructedDataSourceFacet), function (_this3$facetParser$pa) {
                    facets = _this3$facetParser$pa;
                  });
                } else {
                  return _await(_this3.facetParser.parse(oInputData.query, response.data), function (_this3$facetParser$pa2) {
                    facets = _this3$facetParser$pa2;
                  });
                }
              }, function () {
                return _this3.sina._createSearchResultSet({
      ...
      value: function _fireObjectSuggestionsQuery(oInputData) {
        ...
        return _call(function () {
          ...
          return _await(_this4.ajaxClient.getJson(oInputData.url), function (response) {
            ...
            return _await(_this4.itemParser.parse(oInputData.query, response.data), function (searchItems) {
              return _this4.suggestionParser.parseObjectSuggestions(oInputData.query, searchItems);
            });
          });
        });
      }
      ...
      value: function _prepareChartQueryRequest(query, rootCondition, resultDeletion) {
        ...
        var top = query.top || 5; // construct search part of $apply
        ...
        chartOptions.facets = facetScope; // get Query Url
      ...
      value: function executeChartQuery(query) {
        var log = new Log(); // in value help mode delete current condition from root and prepare to construct the value help part of query

        var rootCondition = query.filter.rootCondition.clone();
        var resultDeletion = rootCondition.removeAttributeConditions(query.dimension);

        var url = this._prepareChartQueryRequest(query, rootCondition, resultDeletion); // fire request
      ...
      value: function executeHierarchyQuery(query) {
        ...
        return _call(function () {
          ...
          if (_this5.querySuffix) {
            filter.items.push(_this5.querySuffix);
          } // get Query Url


          var requestUrl = getEshSearchQuery({
            resourcePath: _this5.getPrefix() + "/$all",
            $top: 0,
            searchQueryFilter: filter,
          ...
          return _await(_this5.ajaxClient.getJson(requestUrl), function (response) {
      ...
      value: function executeSuggestionQuery(query) {
        ...
        return _call(function () {
          ...
          return Promise.all([_this6.executeRegularSuggestionQuery(query), _this6.executeObjectSuggestionQuery(query)]).then(function (results) {
            var suggestions = [];
      ...
      value: function executeObjectSuggestionQuery(query) {
        ...
        return _call(function () {
          return _this7.isObjectSuggestionQuery(query) ? _await(_this7._prepareSearchObjectSuggestionRequest(query), function (oUrlData) {
            return _this7._fireObjectSuggestionsQuery(oUrlData);
          }) : Promise.resolve([]);
        });
      }
    }, {
      ...
      value: function _prepareSuggestionQueryRequest(query) {
        ...
        */
        ...
        var suggestionOptions = {
          ...
          $top: top,
          $skip: skip,
          searchQueryFilter: filter // no need to use this.responseAttributes/this.facetAttributes here ($select/facets)
      ...
      value: function _fireSuggestionQuery(query) {
        var url = this._prepareSuggestionQueryRequest(query); // fire request
