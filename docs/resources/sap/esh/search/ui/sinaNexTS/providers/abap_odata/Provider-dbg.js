...
sap.ui.define(["./ajax", "./ajaxTemplates", "./conditionSerializer", "../../core/core", "./dataSourceSerializer", "./labelCalculation", "./suggestionTermSplitter", "../AbstractProvider", "./FacetParser", "./ItemParser", "./NlqParser", "./suggestionParser", "./UserEventLogger", "./MetadataParser", "../../core/errors"], function (___ajax, ajaxTemplates, conditionSerializer, core, dataSourceSerializer, labelCalculation, suggestionTermSplitter, ___AbstractProvider, ___FacetParser, ___ItemParser, ___NlqParser, ___suggestionParser, ___UserEventLogger, ___MetadataParser, ____core_errors) {
  ...
  var Provider = /*#__PURE__*/function (_AbstractProvider) {
    ...
    function Provider() {
      ...
      _defineProperty(_assertThisInitialized(_this2), "id", "abap_odata");
    ...
    _createClass(Provider, [{
      ...
      value: function initAsync(configuration) {
        ...
        return this.loadServerInfo().then(function (serverInfo) {
          _this3.serverInfo = serverInfo.d.results[0];

          if (!_this3.supports("Search")) {
            return Promise.reject(new ESHNotActiveError("Enterprise Search is not active"));
          }

          return _this3.loadBusinessObjectDataSources();
        }).then(function () {
          return {
            capabilities: _this3.sina._createCapabilities({
              fuzzy: false
            })
          };
        });
      ...
      value: function loadServerInfo() {
        ...
        return Promise.all([serverInfosProm, metadataProm]).then(function (values) {
          ...
          if (typeof window !== "undefined") {
            ...
            _this4.serviceXML = dom.window.document;
          }
      ...
      value: function executeSearchQuery(query) {
        ...
        var requestUrl = this.buildQueryUrl(this.requestPrefix, "/SearchQueries"); // fire request
      ...
      value: function executeSuggestionQuery(query) {
        ...
        return _call(function () {
          ...
          return Promise.all([_this.executeRegularSuggestionQuery(query), _this.executeObjectSuggestionQuery(query)]).then(function (results) {
            var suggestions = [];
      ...
      value: function executeObjectSuggestionQuery(query) {
        ...
        requestTemplate.d.QueryOptions.Skip = query.skip;
        requestTemplate.d.QueryOptions.SearchTerms = query.filter.searchTerm;
        this.addSessionId(requestTemplate); // build request url

        var requestUrl = this.buildQueryUrl(this.requestPrefix, "/SuggestionsQueries"); // fire request

        return this.ajaxClient.postJson(requestUrl, requestTemplate).then(function (response) {
          return _this5.suggestionParser.parseObjectSuggestions(query, response.data);
        });
      }
    }, {
      ...
      value: function executeRegularSuggestionQuery(query) {
        ...
        var requestTemplate = JSON.parse(JSON.stringify(ajaxTemplates.suggestionRequest)); // split search term in query into (1) searchTerm (2) suggestionTerm

        var searchTerm = query.filter.searchTerm;
        var splittedTerm = suggestionTermSplitter.split(this, searchTerm); // add search term to condition

        var rootCondition = query.filter.rootCondition.clone(); // assemble request
        ...
        return this.ajaxClient.postJson(requestUrl, requestTemplate).then(function (response) {
          return _this6.suggestionParser.parseRegularSuggestions(query, response.data);
        }).then(function (suggestions) {
          suggestionTermSplitter.concatenate(_this6, splittedTerm, suggestions);
          return suggestions;
        });
      }
    }, {
      ...
      value: function getConfigurationAsync() {
        ...
        return this.ajaxClient.getJson(requestUrl).then(function (response) {
          ...
          requestUrl = _this7.buildQueryUrl(_this7.requestPrefix, "/Users('<current>')");
          return _this7.ajaxClient.getJson(requestUrl).then(function (response) {
      ...
      value: function buildQueryUrl(queryPrefix, queryPostfix) {
        ...
        var systemString = "";
        var systemInRequestUrl = ""; // assign search backend system manuelly
