    ...
    _createClass(FacetParser, [{
      ...
      value: function parse(query, data) {
        ...
        return _call(function () {
          ...
          for (var i = 0; i < value.length; i++) {
            var facetData = value[i]; // var dimension = '';
            // if (query.dimension) {
      ...
      value: function parseDataSourceFacet(query, facetData) {
        ...
        for (var i = 0; i < facetData.Items.length; i++) {
          var cell = facetData.Items[i]; // create filter (used when clicking on the item)
      ...
      value: function formatFacetValue(value
      /**metadata*/
      ) {
        var initialValue = ""; // if (metadata.type === 'Double' || metadata.type === 'Integer') {
        //     initialValue = 0;
        // }
      ...
      value: function parseChartFacet(query, attributeMetadata, facetData) {
        ...
        var dataSource = query.filter.dataSource;
        var items = []; // for search query with attribute facet: create corresponding chart query
