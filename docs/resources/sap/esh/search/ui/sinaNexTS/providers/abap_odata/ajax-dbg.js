    ...
    var defaults = {
      ...
      requestNormalization: function requestNormalization(payload) {
        ...
        if (isSearchRequest(payload) || isNlqSearchRequest(payload) || isChartRequest(payload) || isValueHelperRequest(payload) || isSuggestionRequest(payload) || isObjectSuggestionRequest(payload)) {
          ...
          delete payload.d.QueryOptions.ClientLastExecutionID; // insert "ExcludedDataSources" in payload
          ...
          var headString = '"DataSources":['; // eslint-disable-next-line quotes

          var endString = "]";
          var headIndex = payloadString.indexOf(headString);
          var endIndex = headIndex + payloadString.substring(headIndex).indexOf(endString) + endString.length; // eslint-disable-next-line quotes
