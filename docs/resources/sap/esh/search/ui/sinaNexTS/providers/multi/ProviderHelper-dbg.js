      ...
      value: function updateSuggestionDataSource(results) {
        for (var i = 0; i < results.items.length; i++) {
          ...
          if (item.dataSource) {
            ...
            var multiDataSourceId = this.calculateMultiDataSourceId(item.dataSource.id, item.sina.provider.id); // const multiDataSource = this.createMultiDataSource(multiDataSourceId, item.dataSource);
      ...
      value: function updateDataSourceFacets(resultSetFacets) {
        for (var j = 0; j < resultSetFacets[0].items.length; j++) {
          ...
          if (facetItem.dataSource) {
            var facetItemMultiId = this.calculateMultiDataSourceId(facetItem.dataSource.id, facetItem.sina.provider.id); //new Category, should be insert to multi provider
