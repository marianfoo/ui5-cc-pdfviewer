...
sap.ui.define(["../AbstractProvider", "./FacetMode", "./FederationType", "./ProviderHelper", "../../sina/Sina", "./FederationMethod", "../../core/Log", "../../sina/SinaConfiguration", "../abap_odata/Provider", "../hana_odata/Provider", "../sample/Provider", "../inav2/Provider", "../dummy/Provider", "../../core/errors"], function (___AbstractProvider, ___FacetMode, ___FederationType, ___ProviderHelper, ____sina_Sina, FederationMethod, ____core_Log, ____sina_SinaConfiguration, ___abap_odata_Provider, ___hana_odata_Provider, ___sample_Provider, ___inav2_Provider, ___dummy_Provider, ____core_errors) {
  ...
  var MultiProvider = /*#__PURE__*/function (_AbstractProvider) {
    ...
    function MultiProvider() {
      ...
      _defineProperty(_assertThisInitialized(_this6), "id", "multi");
    ...
    _createClass(MultiProvider, [{
      ...
      value: function initAsync(properties) {
        ...
        return _call(function () {
          _this.log = new Log("MultiProvider");
          _this.sina = properties.sina;
          _this.facetMode = FacetMode[properties.facetMode] || FacetMode.flat;
          _this.federationType = FederationType[properties.federationType] || FederationType.advanced_round_robin;
          _this.multiSina = [];
          _this.multiDataSourceMap = {}; //key: multiId, value: originalDataSource

          _this.sina.dataSourceMap[_this.sina.allDataSource.id] = _this.sina.allDataSource;
          _this.providerHelper = new ProviderHelper(_this);

          switch (_this.federationType) {
            case FederationType.advanced_round_robin:
              {
                _this.federationMethod = new FederationMethod.AdvancedRoundRobin();
                break;
              }

            case FederationType.ranking:
              {
                _this.federationMethod = new FederationMethod.Ranking();
                break;
              }

            case FederationType.round_robin:
              {
                _this.federationMethod = new FederationMethod.RoundRobin();
                break;
              }
          }

          _this.sina.capabilities = _this.sina._createCapabilities({
            fuzzy: false
          });
          ...
          var hasSubProvider = false; // straightforward workaround to use Promise.allSettled() with older Typescript version

          return _await(Promise.allSettled(creationPromises), function (promises) {
            promises.forEach(function (promise) {
              if (promise.status === "rejected") {
                _this.log.warn("Error during creation of subprovider: ".concat(promise.reason.stack));
            ...
            if (!hasSubProvider) {
              _this.log.error("Error during creation of multi provider: no valid subproviders");

              return Promise.reject();
            }

            _this.sina.dataSources.sort(function (a, b) {
      ...
      value: function createAsync(configuration) {
        ...
        return _call(function () {
          _this2.log.debug("Creating new eshclient instance using provider ".concat(configuration.provider));
      ...
      value: function handleAllSearch(query) {
        ...
        return _call(function () {
          ...
          var searchResultSet = _this3.initializeSearchResultSet(query);

          var searchResultSetItemList = []; // search with all dataSource

          searchResultSet.facets.push(_this3.sina._createDataSourceResultSet({
            title: query.filter.dataSource.label,
            items: [],
            query: query
          }));

          for (var i = 0; i < _this3.multiSina.length; i++) {
            childQuery = _this3.multiSina[i].createSearchQuery({
              calculateFacets: query.calculateFacets,
              multiSelectFacets: query.multiSelectFacets,
              dataSource: _this3.multiSina[i].allDataSource,
              searchTerm: query.getSearchTerm(),
              top: query.top,
              skip: query.skip,
              sortOrder: query.sortOrder,
              sina: _this3.multiSina[i]
            });
      ...
      value: function handleUserCategorySearch(query) {
        ...
        return _call(function () {
          ...
          var searchResultSet = _this4.initializeSearchResultSet(query);

          var searchResultSetItemList = []; // search with user defined dataSources (DataSourceType = "UserCategory")

          var myFavorites = query.filter.dataSource;
          var childFavorites = [];

          _this4.multiSina.forEach(function (childSina) {
            ...
            if (childSina.provider.id.startsWith("abap_odata") || childSina.provider.id.startsWith("sample")) {
              ...
              var providerFavorite = _this4.multiDataSourceMap[childFavoritesDataSourceId]; // check if providerFavorite is included in this.multiDataSourceMap
          ...
          myFavorites.subDataSources.forEach(function (subDataSource) {
            ...
            var childDataSourceSina = childDataSource.sina; // abap_odata and sample provider can search with subDataSources, split with each abap_odata and sample provider
          ...
          return Promise.all(queries).then(function (result) {
            ...
            for (var j = 0; j < result.length; j++) {
              ...
              for (var k = 0; k < querySearchResultSet.items.length; k++) {
                ...
                var dataSource = _this4.sina.dataSourceMap[multiId]; // update dataSource consisting of provider Id and dataSource Id
              ...
              searchResultSet.totalCount += querySearchResultSet.totalCount;
              searchResultSetItemList.push(querySearchResultSet.items); // favorite should certainly be a dataSource facet
      ...
      value: function handleBusinessObjectSearch(query) {
        ...
        return _call(function () {
          ...
          var childDataSource = _this5.multiDataSourceMap[query.filter.dataSource.id];
          var rootCondition = query.getRootCondition().clone();

          var searchResultSet = _this5.initializeSearchResultSet(query);

          _this5.providerHelper.updateRootCondition(rootCondition, childDataSource.sina);
          ...
          return _await(childQuery.getResultSetAsync().then(function (querySearchResultSet) {
            ...
            for (var i = 0; i < searchResultSet.items.length; i++) {
              ...
              var resultItemMultiId = _this5.providerHelper.calculateMultiDataSourceId(resultItem.dataSource.id, resultItem.sina.provider.id); //update attributes metadata


              _this5.providerHelper.updateAttributesMetadata(resultItem.dataSource, _this5.sina.dataSourceMap[resultItemMultiId]); //set the facet result item dataSource as multi provider dataSource
            ...
            var multiFacets; //dataSource facet
