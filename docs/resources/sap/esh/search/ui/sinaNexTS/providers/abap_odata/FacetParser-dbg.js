    ...
    _createClass(FacetParser, [{
      ...
      value: function parseDataSourceFacet(query, facetData) {
        ...
        for (var i = 0; i < facetData.Values.results.length; i++) {
          var cell = facetData.Values.results[i]; // create filter (used when clicking on the item)
      ...
      value: function parseChartFacet(query, facetData) {
        ...
        var metadata = dataSource.getAttributeMetadata(attributeId); // for search query with attribute facet: create corresponding chart query
