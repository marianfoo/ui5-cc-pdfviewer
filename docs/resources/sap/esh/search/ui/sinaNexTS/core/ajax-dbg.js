...
sap.ui.define(["./errors", "./util"], function (___errors, ___util) {
  ...
  var _request = _async(function (properties) {
    if (typeof window !== "undefined") {
      return new Promise(function (resolve, reject) {
        // Browser
        // new http request
        var xhttp = new XMLHttpRequest(); // callback handler
        ...
        var url = addEncodedUrlParameters(properties.url, properties.parameters); // write headers to http request
      ...
      var fetch = require("node-fetch"); // eslint-disable-next-line @typescript-eslint/no-var-requires
      ...
      return _catch(function () {
        return _await(fetch(url, config), function (res) {
          if (res.ok) {
            ...
            var _res$statusText = res.statusText,
    ...
    _createClass(Client, [{
      ...
      value: function getXML(url) {
        ...
        return _call(function () {
          var _exit = false;
          var that = _this;
          return that.recordOptions.mode === "none" ? _await(that._client.getXML(url)) : _await(_invoke(function () {
            if (that.recordOptions.mode === "replay") {
              return _await(that._replay(url, null), function (_that$_replay) {
                var _await$that$_replay$d = _that$_replay.data;
                _exit = true;
  ...
  var _Client = /*#__PURE__*/function () {
    ...
    _createClass(_Client, [{
      ...
      value: function _requestWithCsrf(properties, renewCsrf) {
        ...
        return _call(function () {
          var _exit2 = false;
          return addEncodedUrlParameters(_this7.csrfFetchRequest.url, _this7.csrfFetchRequest.parameters) === addEncodedUrlParameters(properties.url, properties.parameters) ? _await(_this7._fetchCsrf()) : _await(_invoke(function () {
            if (renewCsrf && !_this7.csrfToken) {
              return _await(_this7._fetchCsrf(), function () {
                var _this7$_requestWithCs = _this7._requestWithCsrf(properties, false);
            ...
            if (_exit2) return _result2; // do request with csrf token
        ...
        return _call(function () {
          ...
          return _await(_invoke(function () {
            if (!_this8.csrf) {
              ...
              return _await(_request(properties), function (_await$request) {
                _exit3 = true;
                return _await$request;
              });
            }
          }, function (_result3) {
            if (_exit3) return _result3; // if csrf fetch request is not set -> treat first request as csrf fetch request
