...
(function(){
sap.ui.define(["../sina/AttributeSemanticsType", "./errors"], function (___sina_AttributeSemanticsType, ___errors) {
  ...
  function timeoutDecorator(originalFunction, timeout) {
    var decoratedFunction = function decoratedFunction() {
      ...
      var that = this; // eslint-disable-next-line prefer-rest-params
  ...
  function refuseOutdatedResponsesDecorator(originalFunction) {
    ...
    var decoratedFunction = function decoratedFunction() {
      var requestId = ++maxRequestId; // eslint-disable-next-line prefer-rest-params
  ...
  var DelayedConsumer = /*#__PURE__*/function () {
    function DelayedConsumer(properties) {
      ...
      this.timeDelay = properties.timeDelay || 1000; // eslint-disable-next-line @typescript-eslint/no-empty-function
  ...
  function addPotentialNavTargetsToAttribute(resultSet) {
    if (resultSet.items) {
      ...
      for (var i = 0; i < items.length; i++) {
        var item = items[i]; //the idea of nav targets extended to geo data preparation!
